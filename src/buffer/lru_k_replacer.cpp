...
auto LRUKReplacer::Judge(frame_id_t s, frame_id_t t) -> bool {
  ...
  // if (hash_[s].time_.size() == k_ && hash_[t].time_.size() == k_) {
  //   return hash_[s].time_[0] < hash_[t].time_[0];
  // }
  // return hash_[s].time_.back() < hash_[t].time_.back();
  return hash_[s].time_[0] < hash_[t].time_[0];
}
...
void LRUKReplacer::RecordAccess(frame_id_t frame_id) {
  ...
  if (hash_[frame_id].time_.size() == k_) {
    hash_[frame_id].time_.erase(hash_[frame_id].time_.begin());
  }
  hash_[frame_id].time_.emplace_back(current_timestamp_++);
  latch_.unlock();
}
