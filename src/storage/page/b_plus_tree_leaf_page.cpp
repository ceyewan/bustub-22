...
INDEX_TEMPLATE_ARGUMENTS
auto B_PLUS_TREE_LEAF_PAGE_TYPE::Insert(MappingType value, int index,
                                        const KeyComparator &keyComparator)
    -> bool {
  if (index < GetSize() &&
      keyComparator(value.first, array_[index].first) == 0) {
    return false;
  }
...
INDEX_TEMPLATE_ARGUMENTS
auto BPlusTreeLeafPage<KeyType, ValueType, KeyComparator>::Remove(
    const KeyType &key, int index, const KeyComparator &keyComparator) -> bool {
...
INDEX_TEMPLATE_ARGUMENTS
auto BPlusTreeLeafPage<KeyType, ValueType, KeyComparator>::Delete(
    const KeyType &key, const KeyComparator &keyComparator) -> bool {
...
INDEX_TEMPLATE_ARGUMENTS
auto BPlusTreeLeafPage<KeyType, ValueType, KeyComparator>::Merge(
    Page *right_page, BufferPoolManager *buffer_pool_manager_) -> void {
  auto right =
      reinterpret_cast<B_PLUS_TREE_LEAF_PAGE_TYPE *>(right_page->GetData());
...
INDEX_TEMPLATE_ARGUMENTS
auto BPlusTreeLeafPage<KeyType, ValueType, KeyComparator>::InsertFirst(
    const KeyType &key, const ValueType &value) -> void {
...
INDEX_TEMPLATE_ARGUMENTS
auto BPlusTreeLeafPage<KeyType, ValueType, KeyComparator>::InsertLast(
    const KeyType &key, const ValueType &value) -> void {
  array_[GetSize()] = std::make_pair(key, value);
  IncreaseSize(1);
}
INDEX_TEMPLATE_ARGUMENTS
auto BPlusTreeLeafPage<KeyType, ValueType, KeyComparator>::GetPair(int index)
