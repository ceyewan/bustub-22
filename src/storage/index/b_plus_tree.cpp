...
auto BPLUSTREE_TYPE::FindLeafPage(const KeyType &key) -> Page * {
  ...
  while (!curr_page_inter->IsLeafPage()) {
    Page *next_page = buffer_pool_manager_->FetchPage(
        curr_page_inter->Lookup(key, comparator_));
    auto next_page_inter =
        reinterpret_cast<InternalPage *>(next_page->GetData());
...
INDEX_TEMPLATE_ARGUMENTS
auto BPLUSTREE_TYPE::FindLeafPageRW(const KeyType &key,
                                    Transaction *transaction, Operation op)
    -> Page * {
  if (IsEmpty()) {
    return nullptr;
  }
...
auto BPLUSTREE_TYPE::Insert(const KeyType &key, const ValueType &value,
  ...
  bool retcode =
      leaf_node->Insert(std::make_pair(key, value), index, comparator_);
  if (!retcode) {
  ...
  if (leaf_node->GetSize() == leaf_max_size_) {
    ...
    auto leaf_bother_node =
        reinterpret_cast<LeafPage *>(page_bother->GetData());
    leaf_bother_node->Init(page_bother_id, INVALID_PAGE_ID, leaf_max_size_);

    leaf_node->Break(page_bother);

    InsertInParentRW(page_leaf, leaf_bother_node->KeyAt(0), page_bother,
                     transaction);
...
INDEX_TEMPLATE_ARGUMENTS
auto BPLUSTREE_TYPE::InsertInParent(Page *page_leaf, const KeyType &key,
                                    Page *page_bother) -> void {
  ...
  if (tree_page->GetParentPageId() == INVALID_PAGE_ID) {
    ...
    auto page_leaf_node =
        reinterpret_cast<BPlusTreePage *>(page_leaf->GetData());
    page_leaf_node->SetParentPageId(new_page_id);
    auto page_bother_node =
        reinterpret_cast<BPlusTreePage *>(page_bother->GetData());
  ...
  auto page_bother_node =
      reinterpret_cast<InternalPage *>(page_bother->GetData());
  if (parent_node->GetSize() < parent_node->GetMaxSize()) {
    parent_node->Insert(std::make_pair(key, page_bother->GetPageId()),
                        comparator_);
  ...
  Page *page_parent_bother =
      buffer_pool_manager_->NewPage(&page_parent_bother_id);
  auto parent_bother_node =
      reinterpret_cast<InternalPage *>(page_parent_bother->GetData());
  parent_bother_node->Init(page_parent_bother_id, INVALID_PAGE_ID,
                           internal_max_size_);
  parent_node->Break(key, page_bother, page_parent_bother, comparator_,
...
auto BPLUSTREE_TYPE::DeleteEntry(Page *&page, const KeyType &key) -> void {
  ...
  if (root_page_id_ == b_node->GetPageId() && b_node->IsLeafPage() &&
      b_node->GetSize() == 0) {
  ...
  if (root_page_id_ == b_node->GetPageId() && b_node->IsRootPage() &&
      b_node->GetSize() == 1) {
    auto inter_node = reinterpret_cast<InternalPage *>(page->GetData());
  ...
  if (b_node->GetSize() < b_node->GetMinSize()) {
    ...
    auto parent_node = reinterpret_cast<InternalPage *>(parent_page->GetData());
    parent_node->GetBotherPage(page->GetPageId(), bother_page, parent_key,
                               ispre, buffer_pool_manager_);

    auto bother_node =
        reinterpret_cast<BPlusTreePage *>(bother_page->GetData());
    if ((bother_node->GetSize() + b_node->GetSize()) <= GetMaxsize(b_node)) {
      ...
      if (b_node->IsRootPage()) {
        auto inter_bother_node =
            reinterpret_cast<InternalPage *>(bother_page->GetData());
        // auto inter_b_node = reinterpret_cast<InternalPage
        // *>(page->GetData());
        inter_bother_node->Merge(parent_key, page, buffer_pool_manager_);
      } else {
        auto leaf_bother_node =
            reinterpret_cast<LeafPage *>(bother_page->GetData());
      ...
      if (ispre) {
        if (bother_node->IsRootPage()) {
          ...
          page_id_t last_value =
              inter_bother_node->ValueAt(inter_bother_node->GetSize() - 1);
          KeyType last_key =
              inter_bother_node->KeyAt(inter_bother_node->GetSize() - 1);
          ...
          ValueType last_value =
              leaf_bother_node->ValueAt(leaf_bother_node->GetSize() - 1);
          KeyType last_key =
              leaf_bother_node->KeyAt(leaf_bother_node->GetSize() - 1);
        ...
        if (bother_node->IsRootPage()) {
          ...
          inter_b_node->Insert(std::make_pair(parent_key, first_value),
                               comparator_);
...
auto BPLUSTREE_TYPE::Begin() -> INDEXITERATOR_TYPE {
  ...
  while (!curr_page_inter->IsLeafPage()) {
    Page *next_page =
        buffer_pool_manager_->FetchPage(curr_page_inter->ValueAt(0));
    next_page->RLatch();
    auto next_page_inter =
        reinterpret_cast<InternalPage *>(next_page->GetData());
    curr_page->RUnlatch();
  ...
  return INDEXITERATOR_TYPE(curr_page, 0, curr_page->GetPageId(),
                            buffer_pool_manager_);
}
...
auto BPLUSTREE_TYPE::Begin(const KeyType &key) -> INDEXITERATOR_TYPE {
  ...
  return INDEXITERATOR_TYPE(leaf_page, index, leaf_page->GetPageId(),
                            buffer_pool_manager_);
}
...
auto BPLUSTREE_TYPE::End() -> INDEXITERATOR_TYPE {
  ...
  while (!curr_page_inter->IsLeafPage()) {
    Page *next_page = buffer_pool_manager_->FetchPage(
        curr_page_inter->ValueAt(curr_page_inter->GetSize() - 1));
    next_page->RLatch();
    auto next_page_inter =
        reinterpret_cast<InternalPage *>(next_page->GetData());
    curr_page->RUnlatch();
  ...
  curr_page->RUnlatch();
  buffer_pool_manager_->UnpinPage(curr_page->GetPageId(), false);
  return INDEXITERATOR_TYPE(curr_page, curr_node->GetSize(), page_id,
                            buffer_pool_manager_);
}
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
void BPLUSTREE_TYPE::ToGraph(BPlusTreePage *page, BufferPoolManager *bpm,
  ...
  if (page->IsLeafPage()) {
    ...
    if (leaf->GetNextPageId() != INVALID_PAGE_ID) {
      out << leaf_prefix << leaf->GetPageId() << " -> " << leaf_prefix
          << leaf->GetNextPageId() << ";\n";
      out << "{rank=same " << leaf_prefix << leaf->GetPageId() << " "
          << leaf_prefix << leaf->GetNextPageId() << "};\n";
    }

    // Print parent links if there is a parent
    if (leaf->GetParentPageId() != INVALID_PAGE_ID) {
      out << internal_prefix << leaf->GetParentPageId() << ":p"
          << leaf->GetPageId() << " -> " << leaf_prefix << leaf->GetPageId()
          << ";\n";
    }
  } else {
    ...
    if (inner->GetParentPageId() != INVALID_PAGE_ID) {
      out << internal_prefix << inner->GetParentPageId() << ":p"
          << inner->GetPageId() << " -> " << internal_prefix
          << inner->GetPageId() << ";\n";
    }
    // Print leaves
    for (int i = 0; i < inner->GetSize(); i++) {
      auto child_page = reinterpret_cast<BPlusTreePage *>(
          bpm->FetchPage(inner->ValueAt(i))->GetData());
      ToGraph(child_page, bpm, out);
      if (i > 0) {
        auto sibling_page = reinterpret_cast<BPlusTreePage *>(
            bpm->FetchPage(inner->ValueAt(i - 1))->GetData());
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
void BPLUSTREE_TYPE::ToString(BPlusTreePage *page,
                              BufferPoolManager *bpm) const {
  if (page->IsLeafPage()) {
    auto *leaf = reinterpret_cast<LeafPage *>(page);
    std::cout << "Leaf Page: " << leaf->GetPageId()
              << " parent: " << leaf->GetParentPageId()
    ...
    for (int i = 0; i < internal->GetSize(); i++) {
      ToString(reinterpret_cast<BPlusTreePage *>(
                   bpm->FetchPage(internal->ValueAt(i))->GetData()),
               bpm);
    }
  }
  bpm->UnpinPage(page->GetPageId(), false);
...
INDEX_TEMPLATE_ARGUMENTS
auto BPLUSTREE_TYPE::InsertInParentRW(Page *page_leaf, const KeyType &key,
                                      Page *page_bother,
  ...
  parent_node->Break(key, page_bother, page_parent_bother, comparator_,
                     buffer_pool_manager_);
  InsertInParentRW(parent_page, parent_bother_node->KeyAt(0),
                   page_parent_bother, transaction);
  buffer_pool_manager_->UnpinPage(page_parent_bother_id, true);
  buffer_pool_manager_->UnpinPage(parent_id, true);
}
INDEX_TEMPLATE_ARGUMENTS
auto BPLUSTREE_TYPE::DeleteEntryRW(Page *&page, const KeyType &key,
                                   Transaction *transaction) -> void {
  // std::cout << "进入delete递归";
  auto b_node = reinterpret_cast<BPlusTreePage *>(page->GetData());
  if (b_node->IsLeafPage()) {
  ...
  if (b_node->GetSize() < b_node->GetMinSize()) {
    ...
    // auto parent_page = buffer_pool_manager_->FetchPage(parent_page_id);
    auto parent_page =
        (*transaction->GetPageSet())[transaction->GetPageSet()->size() - 2];
    auto parent_node = reinterpret_cast<InternalPage *>(parent_page->GetData());
    // parent_node->GetBotherPageRW(page->GetPageId(), bother_page, parent_key,
    // ispre, buffer_pool_manager_, transaction);
    parent_node->GetBotherPage(page->GetPageId(), bother_page, parent_key,
                               ispre, buffer_pool_manager_);
    auto bother_node =
        reinterpret_cast<BPlusTreePage *>(bother_page->GetData());
